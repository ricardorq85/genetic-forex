...
MAX_TENDENCIA AS (
	SELECT * FROM (
		SELECT TEN.PRECIO_CALCULADO, ROW_NUMBER() OVER (ORDER BY TEN.PRECIO_CALCULADO DESC) RN
		FROM UNION_TENDENCIAS
	) WHERE RN = 1
),
MIN_TENDENCIA AS (
	SELECT * FROM (
		SELECT TEN.PRECIO_CALCULADO, ROW_NUMBER() OVER (ORDER BY TEN.PRECIO_CALCULADO ASC) RN
		FROM UNION_TENDENCIAS
	) WHERE RN = 1
),
MAXMIN_TENDENCIA AS (
	SELECT MAXTENDENCIA.PREDIODO MAXPERIODO, MINTENDENCIA.PERIODO MINPERIODO,
		MAXTENDENCIA.FECHA_TENDENCIA MAXFETENDENCIA, MINTENDENCIA.FECHA_TENDENCIA MINFETENDENCIA,		
		MAXTENDENCIA.PRECIO_CALCULADO MAXPRECIO, MINTENDENICIA.PRECIO_CALCULADO MINPRECIO,
		MAXTENDENCIA.CANTIDAD MAXCANTIDAD, MINTENDENCIA.CANTIDAD MINCANTIDAD
	FROM MAX_TENDENCIA MAXTENDENCIA, MIN_TENDENCIA MINTENDENCIA
),
