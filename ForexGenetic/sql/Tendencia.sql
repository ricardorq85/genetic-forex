...
SELECT TO_CHAR(TEN.FECHA_BASE, 'YYYYMMDD'), COUNT(*), MIN(TEN.FECHA_BASE) MINFEBASE,
		MAX(TEN.FECHA_BASE) MAXFEBASE, MAX(TEN.FECHA) MAXFE,	SUM(TEN.PIPS) PIPS
FROM TENDENCIA TEN --TENDENCIA_ULTIMOSDATOS TEN
...
SELECT TO_CHAR(TEN.FECHA, 'YYYYMMDD') FECHA, COUNT(*), MIN(TEN.FECHA) MINFE,
		MAX(TEN.FECHA) MAXFE
FROM TENDENCIA TEN --TENDENCIA_ULTIMOSDATOS TEN
GROUP BY TO_CHAR(TEN.FECHA, 'YYYYMMDD')
ORDER BY TO_CHAR(TEN.FECHA, 'YYYYMMDD') DESC
--ORDER BY COUNT(*) DESC
;
...
WHERE FECHA_BASE BETWEEN to_date('2016/11/01 01:27', 'YYYY/MM/DD HH24:MI') AND to_date('2016/12/01 01:27', 'YYYY/MM/DD HH24:MI')
ORDER BY FECHA DESC
;
