SELECT TO_CHAR(TEN.FECHA_BASE, 'YYYYMMDD') dia_fechabase, COUNT(*), MIN(TEN.FECHA_BASE) MINFEBASE,
		MAX(TEN.FECHA_BASE) MAXFEBASE, MAX(TEN.FECHA) MAXFE,	SUM(TEN.PIPS) PIPS
FROM TENDENCIA TEN --
--TENDENCIA_ULTIMOSDATOS TEN
WHERE TO_CHAR(TEN.FECHA_BASE, 'YYYY') IN('2018','2019')
GROUP BY TO_CHAR(TEN.FECHA_BASE, 'YYYYMMDD')
ORDER BY TO_CHAR(TEN.FECHA_BASE, 'YYYYMMDD') DESC
--ORDER BY MAX(TEN.FECHA) DESC, TO_CHAR(TEN.FECHA_BASE, 'YYYYMMDD') DESC
;

SELECT * FROM TENDENCIA T
--WHERE T.ID_INDIVIDUO='1394755200000.32'
where T.FECHA_BASE>TO_DATE('2019/01/01 00:00','YYYY/MM/DD HH24:MI')
--ORDER BY FECHA_BASE ASC
ORDER BY FECHA_BASE aSC
;

SELECT * FROM TENDENCIA --TENDENCIA_ULTIMOSDATOS
WHERE FECHA>SYSDATE-1
ORDER BY FECHA deSC
--ORDER BY FECHA_BASE ASC
;

SELECT TO_CHAR(TEN.FECHA, 'YYYYMMDD HH24:MI'), COUNT(*)
FROM FOREX.TENDENCIA_ULTIMOSDATOS TEN
--WHERE --TEN.FECHA_BASE<=TO_DATE('2016/12/21 23:59', 'YYYY/MM/DD HH24:MI')
--TRUNC(TEN.FECHA_BASE)=TO_DATE('2017/12/05 00:00', 'YYYY/MM/DD HH24:MI')
GROUP BY TO_CHAR(TEN.FECHA, 'YYYYMMDD HH24:MI')
ORDER BY TO_CHAR(TEN.FECHA, 'YYYYMMDD HH24:MI') DESC;

SELECT * FROM TENDENCIA 
WHERE FECHA_BASE>TO_DATE('2016/12/01 00:00', 'YYYY/MM/DD HH24:MI')
ORDER BY FECHA_BASE ASC
;

SELECT * FROM TENDENCIA 
WHERE FECHA_BASE BETWEEN TO_DATE('2018/05/01 00:03', 'YYYY/MM/DD HH24:MI')
	AND TO_DATE('2018/06/01 00:03', 'YYYY/MM/DD HH24:MI')
--ORDER BY FECHA_BASE ASC
ORDER BY FECHA DESC
;

SELECT MAX(FECHA) FROM TENDENCIA 
WHERE FECHA_BASE BETWEEN to_date('2018/02/05 18:19', 'YYYY/MM/DD HH24:MI') AND to_date('2018/03/05 18:19', 'YYYY/MM/DD HH24:MI')
--ORDER BY FECHA DESC
;
SELECT TO_CHAR(TEN.FECHA_BASE, 'YYYYMMDD HH24'), COUNT(*), SUM(TEN.PIPS) PIPS, MAX(FECHA)
...
SELECT TO_CHAR(TEN.FECHA_BASE, 'YYYYMM'), COUNT(*),
	MIN(TEN.FECHA_BASE) MINFEBASE, MAX(TEN.FECHA_BASE) MAXFEBASE, SUM(TEN.PIPS) PIPS, MAX(FECHA)
FROM FOREX.TENDENCIA TEN
...
SELECT T.FECHA_BASE, T.FECHA_TENDENCIA
FROM TENDENCIA T	
WHERE T.FECHA_BASE=TO_DATE('2016/03/18 22:10','YYYY/MM/DD HH24:MI')
...
SELECT TRUNC(T.FECHA_TENDENCIA, 'HH24') FECHA_TENDENCIA, MIN(T.FECHA_BASE), Max(T.FECHA_BASE),
	AVG(T.PRECIO_CALCULADO), COUNT(*)
FROM TENDENCIA T
WHERE T.FECHA_BASE BETWEEN ADD_MONTHS(TO_DATE('2016/03/26 00:00','YYYY/MM/DD HH24:MI'),-1)
		AND TO_DATE('2016/03/26 00:00','YYYY/MM/DD HH24:MI')
--T.FECHA_BASE <= TO_DATE('2016/03/26 00:00','YYYY/MM/DD HH24:MI')		
AND TRUNC(T.FECHA_TENDENCIA, 'HH24')=TO_DATE('2016/04/06 01:00','YYYY/MM/DD HH24:MI')
GROUP BY TRUNC(T.FECHA_TENDENCIA, 'HH24')
--ORDER BY T.FECHA_TENDENCIA ASC
ORDER BY COUNT(*) aSC, MAX(T.FECHA_BASE) DESC, MIN(T.FECHA_BASE) DESC
;

SELECT IND.ID, IND.TIPO_OPERACION, T.FECHA_BASE, T.PRECIO_BASE,	T.FECHA_TENDENCIA, T.PIPS, T.PIPS_ACTUALES, T.PRECIO_CALCULADO,
T.*, TO_CHAR(T.FECHA, 'YYYY/MM/DD HH24:MI:SS')
FROM TENDENCIA T INNER JOIN INDIVIDUO IND ON IND.ID=T.ID_INDIVIDUO
WHERE T.TIPO_TENDENCIA='BUY_SELL_20170204-2'
--AND T.FECHA_TENDENCIA=TO_DATE('2017.01.17 02:15', 'YYYY/MM/DD HH24:MI')
AND T.FECHA_BASE>=TO_DATE('2017/11/24 01:31', 'YYYY/MM/DD HH24:MI')
ORDER BY T.FECHA aSC, T.FECHA_BASE DESC, T.FECHA_TENDENCIA asc, T.TIPO_TENDENCIA DESC
