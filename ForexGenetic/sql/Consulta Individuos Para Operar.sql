...
SELECT FILT.ID_INDIVIDUO,
	...
	FROM (SELECT FILTERED.ID_INDIVIDUO,
				--NVL(MIN(FILTERED.PIPS_SEMANA),0) CRITERIO_ORDER1,
				NVL(MIN(FILTERED.EOP_PIPS_TOTALES),0) CRITERIO_ORDER1,
			...
			WHERE FILTERED.CANTIDAD/((FILTERED.FECHA_FINAL-FILTERED.FECHA_INICIAL)/30)<10
				AND FILTERED.EOP_PIPS_TOTALES>0
				AND FILTERED.PIPS_TOTALES_PARALELAS>FILTERED.EOP_PIPS_TOTALES
				AND FILTERED.PIPS_TOTALES_PARALELAS>1000
				--AND (FILTERED.PIPS_AGRUPADO_MINUTOS>1000)	AND FILTERED.PIPS_AGRUPADO_HORAS>0 AND FILTERED.PIPS_AGRUPADO_DIAS>0)
				AND (FILTERED.CANTIDAD_PARALELAS/FILTERED.CANTIDAD)<10	  
				AND (FILTERED.CANTIDAD_PARALELAS/FILTERED.CANTIDAD)>1
				AND FILTERED.CANTIDAD_INDIVIDUOS>1
				AND FILTERED.CANTIDAD/((FILTERED.FECHA_FINAL-FILTERED.FECHA_INICIAL)/30)>1
				AND (FILTERED.EOP_PIPS_TOTALES/FILTERED.CANTIDAD)>200
				--AND (FILTERED.PIPS_TOTALES_PARALELAS/FILTERED.CANTIDAD_PARALELAS)>200
				AND (FILTERED.FECHA_FINAL-FILTERED.FECHA_INICIAL)>21
		...
		SELECT FILTERED.ID_INDIVIDUO,
			...
			WHERE FILTERED.CANTIDAD/((FILTERED.FECHA_FINAL-FILTERED.FECHA_INICIAL)/30)<10
				AND FILTERED.EOP_PIPS_TOTALES>1000 
				AND FILTERED.PIPS_TOTALES_PARALELAS>FILTERED.EOP_PIPS_TOTALES
				AND FILTERED.PIPS_TOTALES_PARALELAS>3000
				--AND (FILTERED.PIPS_AGRUPADO_MINUTOS>1000 AND FILTERED.PIPS_AGRUPADO_HORAS>1000 AND FILTERED.PIPS_AGRUPADO_DIAS>1000)
				AND (FILTERED.CANTIDAD_PARALELAS/FILTERED.CANTIDAD)<10
				AND (FILTERED.CANTIDAD_PARALELAS/FILTERED.CANTIDAD)>1
				AND FILTERED.CANTIDAD_INDIVIDUOS>1
				AND FILTERED.CANTIDAD/((FILTERED.FECHA_FINAL-FILTERED.FECHA_INICIAL)/30)>1
...
--				SELECT FILTERED.ID_INDIVIDUO,
--				NVL(MIN(FILTERED.PIPS_SEMANA),0) CRITERIO_ORDER1,
--				NVL(MIN(FILTERED.EOP_PIPS_TOTALES),0) CRITERIO_ORDER2,
--				ROUND((NVL(MIN(FILTERED.PIPS_MES),0)+NVL(MIN(FILTERED.PIPS_ANYO),0)+MIN(PIPS_TOTALES))/3) CRITERIO_ORDER2,
				--ROUND((NVL(MIN(FILTERED.PIPS_ANYO),0)+NVL(MIN(FILTERED.PIPS_MES),0)+NVL(MIN(FILTERED.PIPS_ANYO),0))/3) CRITERIO_ORDER2,
--				FILTERED.FECHA_SEMANA VIGENCIA1,
--				MAX(FILTERED.FECHA) FECHA_ORDER1, MAX(FILTERED.FECHA_FINAL) FECHA_ORDER2
...
--			WHERE --FILTERED.CANTIDAD/((FILTERED.FECHA_FINAL-FILTERED.FECHA_INICIAL)/30)<10
				--AND 
--				FILTERED.PIPS_TOTALES<=1000
--				AND FILTERED.PIPS_TOTALES_PARALELAS>100000000
--				AND (FILTERED.PIPS_AGRUPADO_MINUTOS>0 AND FILTERED.PIPS_AGRUPADO_HORAS>0 AND FILTERED.PIPS_AGRUPADO_DIAS>0)
--				AND FILTERED.CANTIDAD_INDIVIDUOS>1
--				AND (FILTERED.PIPS_TOTALES_PARALELAS/FILTERED.CANTIDAD_PARALELAS)>200
--		GROUP BY FILTERED.ID_INDIVIDUO, FILTERED.FECHA_SEMANA
	) FILT
