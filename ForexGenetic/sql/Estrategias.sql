SELECT PERI.ID, PERI.PIPS_TOTALES, PERI.PIPS_TOTALES_PARALELAS, PERI.TIPO_OPERACION, PERI.*
FROM ESTRATEGIA_OPERACION_PERIODO PERI
WHERE PERI.ID>=1250470
ORDER BY PERI.ID DESC;
...
WHERE ESTRATEGIA_PERIODO IN(1293097)
--AND ID_INDIVIDUO='1454889600000.364'
ORDER BY FECHA_APERTURA ASC;

SELECT EOP.ID_INDIVIDUO, COUNT(*), SUM(EOP.PIPS) 
FROM OPERACION_ESTRATEGIA_PERIODO EOP
WHERE ESTRATEGIA_PERIODO IN(1293097)
GROUP BY EOP.ID_INDIVIDUO
ORDER BY SUM(EOP.PIPS) ASC;

SELECT EOP.ID, EOP.* 
FROM ESTRATEGIA_OPERACION_PERIODO EOP
WHERE TIPO_OPERACION='SELL'
AND PIPS_TOTALES>0 AND PIPS_TOTALES_PARALELAS>0
--AND (FECHA_FINAL-FECHA_INICIAL)<60
--WHERE FECHA>TRUNC(SYSDATE)
ORDER BY EOP.FECHA DESC, 
EOP.PIPS_TOTALES DESC, EOP.ID DESC;

SELECT EOP.TIPO_OPERACION TIPO, TO_CHAR(FECHA, 'YYYYMMDD') FE, MIN(ID), MAX(ID), COUNT(*),
  MIN(FECHA), MAX(FECHA), MIN(PIPS_TOTALES), MAX(PIPS_TOTALES)
FROM FOREX.ESTRATEGIA_OPERACION_PERIODO EOP
GROUP BY EOP.TIPO_OPERACION, TO_CHAR(FECHA, 'YYYYMMDD')
ORDER BY MAX(ID) DESC
;

SELECT TO_CHAR(FECHA, 'YYYYMMDD') FE, SUM(PIPS_TOTALES), COUNT(*) 
FROM ESTRATEGIA_OPERACION_PERIODO EOP
--WHERE CANTIDAD>0 AND PIPS_TOTALES>0
GROUP BY TO_CHAR(FECHA, 'YYYYMMDD')
ORDER BY TO_CHAR(FECHA, 'YYYYMMDD') DESC;

SELECT TO_CHAR(FECHA_FINAL, 'YYYYMM') FE, SUM(PIPS_TOTALES), COUNT(*) 
FROM ESTRATEGIA_OPERACION_PERIODO EOP
--WHERE CANTIDAD>0 AND PIPS_TOTALES>0
GROUP BY TO_CHAR(FECHA_FINAL, 'YYYYMM')
ORDER BY TO_CHAR(FECHA_FINAL, 'YYYYMM') DESC;
  
SELECT EOP.TIPO_OPERACION TIPO, FECHA_INICIAL, FECHA_FINAL, MIN(ID), MAX(ID), COUNT(*),
  MIN(FECHA), MAX(FECHA), MIN(PIPS_TOTALES), MAX(PIPS_TOTALES)
FROM FOREX.ESTRATEGIA_OPERACION_PERIODO EOP
...
SELECT 
  MIN(FILTRO_PIPS_X_SEMANA) MIN_FILTRO_PIPS_SEMANA, MIN(FILTRO_PIPS_X_MES) MIN_FILTRO_PIPS_MES,
  MIN(FILTRO_PIPS_X_ANYO) MIN_FILTRO_PIPS_ANYO, MIN(FILTRO_PIPS_TOTALES) MIN_FILTRO_PIPS_TOTALES,
  MAX(FILTRO_PIPS_X_SEMANA) MAX_FILTRO_PIPS_SEMANA, MAX(FILTRO_PIPS_X_MES) MAX_FILTRO_PIPS_MES,
  MAX(FILTRO_PIPS_X_ANYO) MAX_FILTRO_PIPS_ANYO, MAX(FILTRO_PIPS_TOTALES) MAX_FILTRO_PIPS_TOTALES
FROM ESTRATEGIA_OPERACION_PERIODO EOP 
WHERE EOP.PIPS_TOTALES>1000 AND EOP.PIPS_TOTALES_PARALELAS>3000
;
...
SELECT PTFS.ID_INDIVIDUO, PTFS.FECHA_SEMANA
  FROM PREVIO_TOFILESTRING PTFS
	WHERE
		( NVL(PTFS.PIPS_SEMANA,0)>-1500 
    AND PTFS.PIPS_MES>-1000
		AND PTFS.PIPS_ANYO>2800
    AND PTFS.PIPS_TOTALES>12000
    )      AND 
    --PTFS.TIPO_OPERACION='BUY'    AND 
      PTFS.FECHA_SEMANA BETWEEN TO_DATE('2016/09/01 00:00', 'YYYY/MM/DD HH24:MI') 
...
SELECT EOP.ID, EOP.FILTRO_R2_ANYO, EOP.FILTRO_R2_TOTALES, EOP.* 
FROM ESTRATEGIA_OPERACION_PERIODO EOP
WHERE TIPO_OPERACION='SELL' AND EOP.PIPS_TOTALES>0
--EOP.FILTRO_R2_ANYO>0 AND EOP.FILTRO_R2_ANYO<0.01
ORDER BY --EOP.PIPS_TOTALES DESC
EOP.PIPS_TOTALES_PARALELAS DESC
;

SELECT EOP.*
FROM FILTERED_EOP EOP
	INNER JOIN OPERACION_ESTRATEGIA_PERIODO OEP 
		ON EOP.ID=OEP.ESTRATEGIA_PERIODO AND OEP.FECHA_APERTURA>EOP.FECHA_FINAL
;

SELECT EOP.FIRST_ORDER, COUNT(*)
FROM ESTRATEGIA_OPERACION_PERIODO EOP 
WHERE PIPS_TOTALES>1000 AND PIPS_TOTALES_PARALELAS>1000
--AND EOP.PIPS_TOTALES_PARALELAS>EOP.PIPS_TOTALES
--AND TIPO_OPERACION='SELL'
GROUP BY EOP.FIRST_ORDER;
