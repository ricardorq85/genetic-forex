...
FROM INDIVIDUO IND
  INNER JOIN (SELECT OPER.ID_INDIVIDUO, MIN(OPER.PIPS) PIPS_MINIMOS, MAX(OPER.PIPS) PIPS_MAXIMOS, AVG(OPER.PIPS) PIPS_PROMEDIO, STATS_MODE(PIPS) PIPS_MODA,
    ...
    COUNT(*) CANTIDAD
    FROM OPERACION OPER WHERE TIPO='SELL' 
    AND OPER.FECHA_CIERRE IS NOT NULL AND OPER.FECHA_CIERRE < (SELECT TO_DATE(VALOR, 'YYYY/MM/DD HH24:MI') FROM PARAMETRO WHERE NOMBRE='FECHA_ESTADISTICAS')
    AND (OPER.FECHA_CIERRE-OPER.FECHA_APERTURA)*24*60>=NVL((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='DURACION_ESTADISTICAS'),0)
    AND ((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS') IS NULL OR
      (OPER.PIPS<0 AND (SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')/1>0 
      AND OPER.MAX_PIPS_RETROCESO>=NVL((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS'),0)) 
      OR (OPER.PIPS>0 AND (SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')/-1>0 
      AND OPER.MAX_PIPS_RETROCESO<=NVL((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS'),0))
      OR ((OPER.PIPS<0 AND (SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')/1<0 
      AND OPER.PIPS<=NVL((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS'),0)))
      OR ((OPER.PIPS>0 AND (SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')/-1<0 
      AND OPER.PIPS>=NVL((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS'),0))))
  ...
  LEFT JOIN (SELECT P.ID_INDIVIDUO, SUM(P.PIPS) PIPS, MIN(P.PIPS) PIPS_MINIMOS, MAX(P.PIPS) PIPS_MAXIMOS, AVG(P.PIPS) PIPS_PROMEDIO, STATS_MODE(P.PIPS) PIPS_MODA,
    ...
    COUNT(*) CANTIDAD
    FROM OPERACION P 
    WHERE TIPO='SELL' AND P.PIPS>0 
    AND P.FECHA_CIERRE IS NOT NULL AND P.FECHA_CIERRE < (SELECT TO_DATE(VALOR, 'YYYY/MM/DD HH24:MI') FROM PARAMETRO WHERE NOMBRE='FECHA_ESTADISTICAS')    
    AND (P.FECHA_CIERRE-P.FECHA_APERTURA)*24*60>=NVL((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='DURACION_ESTADISTICAS'),0)
    AND (((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS') IS NULL)
     OR ((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')>0
      AND (P.PIPS>=(SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')))
     OR ((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')<0
      AND P.MAX_PIPS_RETROCESO<=NVL((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS'),0)))
  ...
  LEFT JOIN (SELECT N.ID_INDIVIDUO, SUM(N.PIPS) PIPS, MIN(N.PIPS) PIPS_MINIMOS, MAX(N.PIPS) PIPS_MAXIMOS, AVG(N.PIPS) PIPS_PROMEDIO, STATS_MODE(N.PIPS) PIPS_MODA,
    ...
    MIN(MAX_PIPS_RETROCESO) PIPS_RETROCESO_MINIMO, MAX(MAX_PIPS_RETROCESO) PIPS_RETROCESO_MAXIMO, AVG(MAX_PIPS_RETROCESO) PIPS_RETROCESO_PROMEDIO, STATS_MODE(MAX_PIPS_RETROCESO) PIPS_RETROCESO_MODA,    
    COUNT(*) CANTIDAD
    FROM OPERACION N WHERE TIPO='SELL' AND N.PIPS<=0 
    AND N.FECHA_CIERRE IS NOT NULL AND N.FECHA_CIERRE < (SELECT TO_DATE(VALOR, 'YYYY/MM/DD HH24:MI') FROM PARAMETRO WHERE NOMBRE='FECHA_ESTADISTICAS')
    AND (N.FECHA_CIERRE-N.FECHA_APERTURA)*24*60>=NVL((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='DURACION_ESTADISTICAS'),0)
    AND (((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS') IS NULL)
      OR ((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')<0
        AND(N.PIPS<=(SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')))
     OR ((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')>0
      AND (N.MAX_PIPS_RETROCESO>=NVL((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS'),0))))
...
SELECT *
    ...
    AND ((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS') IS NULL OR
      ...
      OR ((OPER.PIPS>0 AND (SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')/-1<0 
      AND OPER.PIPS>=NVL((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS'),0))))
      AND OPER.ID_INDIVIDUO='1328236563095.2'
ORDER BY OPER.MAX_PIPS_RETROCESO DESC
...
SELECT *
    ...
    AND (((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS') IS NULL)
     ...
     OR ((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')<0
      AND P.MAX_PIPS_RETROCESO<=NVL((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS'),0)))  
      AND P.ID_INDIVIDUO='1328236563095.2'     
...
SELECT *
    ...
    AND (((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS') IS NULL)
     ...
     OR ((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS')>0
      AND (N.MAX_PIPS_RETROCESO>=NVL((SELECT TO_NUMBER(VALOR) FROM PARAMETRO WHERE NOMBRE='RETROCESO_ESTADISTICAS'),0))))
  AND N.ID_INDIVIDUO='1328236563095.2'     
  ORDER BY PIPS DESC
;     