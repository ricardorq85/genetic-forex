...
SELECT TIPO, COUNT(*)
 FROM FOREX.OPERACION OPER 
--WHERE OPER.FECHA_APERTURA>TO_DATE('20170206 15:00', 'YYYYMMDD HH24:MI')
GROUP BY TIPO
;

SELECT OPER.ID_INDIVIDUO, OPER.FECHA_APERTURA, OPER.FECHA_CIERRE, OPER.OPEN_PRICE, OPER.SPREAD, 
				OPER.LOTE, OPER.PIPS, OPER.TIPO, OPER.TAKE_PROFIT, OPER.STOP_LOSS, OPER.MAX_PIPS_RETROCESO
				 FROM OPERACION OPER 
				 INNER JOIN PROCESO PROC ON PROC.ID_INDIVIDUO=OPER.ID_INDIVIDUO AND (PROC.FECHA_HISTORICO>=TO_DATE('2016/11/18 23:59','YYYY/MM/DD HH24:MI')) 
				 INNER JOIN INDIVIDUO_INDICADORES IND ON IND.ID=OPER.ID_INDIVIDUO-- AND IND.TIPO_INDIVIDUO='INDICADORES' 
				 WHERE OPER.FECHA_APERTURA<TO_DATE('2016/11/18 23:59','YYYY/MM/DD HH24:MI')	AND (OPER.FECHA_CIERRE IS NULL	OR OPER.FECHA_CIERRE>TO_DATE('2016/11/18 23:59','YYYY/MM/DD HH24:MI')) 
				 AND OPER.FECHA_APERTURA>TO_DATE('2016/11/18 23:59','YYYY/MM/DD HH24:MI')-15 
				 --AND NOT EXISTS (SELECT 1 FROM TENDENCIA T WHERE T.ID_INDIVIDUO=OPER.ID_INDIVIDUO AND T.FECHA_BASE=TO_DATE('2016/11/18 23:59','YYYY/MM/DD HH24:MI') AND T.TIPO_CALCULO='DEFAULT')
				AND OPER.ID_INDIVIDUO in('1394755200000.149')
				;
--1454889600000.1019, pips actuales=400

SELECT * FROM OPERACION OPER
WHERE OPER.FECHA_APERTURA>TO_DATE('2016/11/30 23:59','YYYY/MM/DD HH24:MI')
AND (OPER.FECHA_CIERRE IS NULL OR OPER.FECHA_CIERRE>TO_DATE('2016/12/01 16:59','YYYY/MM/DD HH24:MI'))
ORDER BY OPER.FECHA_APERTURA ASC;

SELECT ROWID, OPER.* FROM OPERACION OPER
WHERE OPER.ID_INDIVIDUO LIKE '1477365411387.2779'
--'1394841600000.83'
--AND OPER.FECHA_APERTURA>=TO_DATE('20180401','YYYYMMDD')
...
SUM(PIPS), COUNT(*) 
FROM OPERACION OPER
WHERE OPER.FECHA_APERTURA>=TO_DATE('20180101','YYYYMMDD') 
AND OPER.ID_INDIVIDUO='1394755200000.158'
GROUP BY TO_CHAR(FECHA_APERTURA, 'YYYYMM')--, OPER.TIPO
ORDER BY TO_CHAR(FECHA_APERTURA, 'YYYYMM') ASC--, OPER.TIPO ASC
;
...
SELECT PIPS, MAX_PIPS_RETROCESO, MAX_VALUE_RETROCESO, MAX_FECHA_RETROCESO, OPER.* FROM OPERACION OPER
WHERE FECHA>TO_DATE('20170116 13:30', 'YYYYMMDD HH24:MI');

select * from detalle_estadisticas where id_individuo='1394841600000.83';