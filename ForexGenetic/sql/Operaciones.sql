...
WHERE ID_INDIVIDUO IN ('1462031621723.723')
AND OPER.FECHA_APERTURA>=TO_DATE('20110101','YYYYMMDD')
ORDER BY FECHA_APERTURA ASC;

SELECT ID_INDIVIDUO, COUNT(*), SUM(PIPS)
 FROM FOREX.OPERACION OPER
WHERE OPER.ID_INDIVIDUO IN ('1454889600000.1202')
--AND TO_CHAR(OPER.FECHA_CIERRE, 'YYYYMM')='201411'
--AND OPER.FECHA_CIERRE<=TO_DATE('20090629','YYYYMMDD')
...
WHERE IND.TIPO_OPERACION='BUY' --AND IND.ID LIKE '1462061850021.%'
GROUP BY ID_INDIVIDUO, 
...
SELECT ID_INDIVIDUO, TO_CHAR(FECHA_CIERRE, 'YYYYMMDD') DIA, COUNT(*), SUM(PIPS)
 FROM FOREX.OPERACION OPER
...
SELECT ID_INDIVIDUO, TO_CHAR(FECHA_CIERRE, 'YYYYMM') FE, COUNT(*), SUM(PIPS)
 FROM FOREX.OPERACION OPER
WHERE OPER.ID_INDIVIDUO like '1453687605734.5'
...
SELECT ID_INDIVIDUO, TO_CHAR(FECHA_CIERRE, 'YYYY') FE, COUNT(*), SUM(PIPS)
 FROM FOREX.OPERACION OPER
WHERE 
ID_INDIVIDUO LIKE '1461512685034.1045'
AND FECHA_CIERRE IS NOT NULL
...
ORDER BY 
TO_CHAR(FECHA_APERTURA, 'YYYYMM') ASC
--SUM(OPER.PIPS) DESC
...
FROM FOREX.OPERACION WHERE ID_INDIVIDUO IN ('1330999046124.191')
GROUP BY ID_INDIVIDUO;
...
SELECT COUNT(*) FROM FOREX.OPERACION OP INNER JOIN FOREX.PROCESO PRO ON PRO.ID_INDIVIDUO=OP.ID_INDIVIDUO
   WHERE OP.FECHA IS NULL 
   --   AND ROWNUM<1000
   ;
 
 SELECT ROUND(AVG(OPER.FECHA_CIERRE-OPER.FECHA_APERTURA)*24*60) FROM OPERACION OPER
 WHERE ID_INDIVIDUO = '1453128626781.100';
