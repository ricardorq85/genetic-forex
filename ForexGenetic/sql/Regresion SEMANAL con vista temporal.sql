...
FROM (
  ...
  FROM (
  SELECT OPER.ID_INDIVIDUO, TRUNC((OPER.FECHA_CIERRE),'IW') FECHA_SEMANA, OPER.FECHA_CIERRE,
    ...
    WHERE OPER.FECHA_CIERRE IS NOT NULL AND OPER.ID_INDIVIDUO='1462763219878.287'
    ORDER BY OPER.FECHA_CIERRE ASC
  ) AC ORDER BY AC.FECHA_CIERRE ASC) AC2
  ...
  WHERE --NOT EXISTS (SELECT 1 FROM OPERACION_X_SEMANA OPC WHERE OPC.ID_INDIVIDUO=AC2.ID_INDIVIDUO)
    ...
    AND AC2.FECHA_SEMANA>TO_DATE('2014/10/13', 'YYYY/MM/DD')
