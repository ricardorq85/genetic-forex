...
PARAMETROS AS (
		SELECT
		'XD' PERIODO, (1440*10) TIEMPO_TENDENCIA,
		TO_DATE(
		'
		2016.12.14 18:08
		'
...
--SELECT PARAM.PERIODO PERIODO, (TEN.CANTIDAD-TEN.MAYORQ_EXIGIDA) DIFFCANT, TEN.*, ROUND(TEN.CANTIDAD/PROM.AVGCANTIDAD,2) FACTORCANTIDAD FROM PARAMETROS PARAM, PROMEDIOS PROM, TENDENCIA_CALCULADA TEN	ORDER BY TEN.FECHA_TENDENCIA ASC;
SELECT PARAM.PERIODO PERIODO, REG.*  FROM PARAMETROS PARAM, REGRESION REG
;

WITH 
PARAMETROS AS (
		SELECT 'XD' PERIODO, (1440*10) TIEMPO_TENDENCIA,
		TO_DATE(
		'
		2017/05/01 07:59
		'
