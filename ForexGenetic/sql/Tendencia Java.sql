...
TENDENCIA_CALCULADA AS (SELECT PARAM.FECHA_PROCESO FECHA_BASE,TRUNC(TEN.FECHA_TENDENCIA, 'HH24') FECHA_TENDENCIA,
			...
			MIN(TEN.PRECIO_CALCULADO) MINPRECIO_CALCULADO, MAX(TEN.PRECIO_CALCULADO) MAXPRECIO_CALCULADO, MAX(FECHA) MAXFE 
	FROM PARAMETROS PARAM, TENDENCIA TEN --TENDENCIA_ULTIMOSDATOS TEN 
	WHERE TEN.TIPO_TENDENCIA=PARAM.TIPO_TENDENCIA AND 
			WEEK_MINUTES(TEN.FECHA_TENDENCIA,TEN.FECHA_BASE)<PARAM.TIEMPO_TENDENCIA
...
SELECT PARAM.PERIODO PERIODO, PRITEN.PRECIO_CALCULADO PRIMERA_TENDENCIA, REG.*  
