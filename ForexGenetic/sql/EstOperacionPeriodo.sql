...
--  PERI.ID, PTFS.ID_INDIVIDUO, PTFS.FECHA_SEMANA,
...
--  PERI.PIPS_TOTALES, PERI.PIPS_TOTALES_PARALELAS
  FROM PREVIO_TOFILESTRING PTFS
	INNER JOIN ESTRATEGIA_OPERACION_PERIODO PERI
      ...
      --AND PTFS.FECHA_SEMANA BETWEEN ADD_MONTHS(PERI.FECHA_FINAL,-1) AND ADD_MONTHS(PERI.FECHA_FINAL,1)
      AND PTFS.PENDIENTE_MES>0 AND PTFS.PENDIENTE_ANYO>0 AND PTFS.PENDIENTE_CONSOL>0 --AND PTFS.PENDIENTE_SEMANA>0
  WHERE 
  (PERI.PIPS_TOTALES>2000 AND 
  ...
  --AND PTFS.ID_INDIVIDUO='1462031621723.649'
