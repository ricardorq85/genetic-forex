...
SELECT PTFS.FECHA_SEMANA, ROUND(MAX(R2_SEMANA),5) R2_SEMANA, ROUND(MAX(R2_MES),5) R2_MES, 
  ROUND(MAX(R2_ANYO),5) R2_ANYO, ROUND(MAX(R2_CONSOL),5) R2_CONSOL
FROM PREVIO_TOFILESTRING PTFS
GROUP BY PTFS.FECHA_SEMANA
ORDER BY PTFS.FECHA_SEMANA DESC
--ORDER BY MAX(PTFS.R2_CONSOL) DESC
;
...
SELECT * FROM PREVIO_TOFILESTRING OPER
WHERE OPER.ID_INDIVIDUO LIKE '1461280102688.61'
ORDER BY OPER.FECHA_SEMANA DESC;
...
WHERE 
OPER.ID_INDIVIDUO LIKE '1461280102688.61'
--AND 
--FECHA_SEMANA>=TO_DATE('20160901', 'YYYYMMDD')
