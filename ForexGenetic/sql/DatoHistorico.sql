SELECT (DH.OPEN+DH.LOW+DH.CLOSE+DH.CLOSE)/4 PRICE, DH.* 
FROM DATOHISTORICO DH 
WHERE DH.FECHA>=TO_DATE('2019/01/01 00:00', 'YYYY/MM/DD HH24:MI')
...
SELECT DH.*
FROM DATOHISTORICO DH 
WHERE AVERAGE>0 AND BOLLINGER_UPPER240>AVERAGE*10;

SELECT 'ROJA' VELA, COUNT(*) CANT, AVG(OPEN-CLOSE) DIFF FROM DATOHISTORICO WHERE OPEN>CLOSE
UNION ALL
SELECT 'VERDE' VELA, COUNT(*) CANT, AVG(CLOSE-OPEN) DIFF FROM DATOHISTORICO WHERE OPEN<CLOSE;

SELECT TRUNC(FECHA, 'MM'), COUNT(*) FROM DATOHISTORICO
WHERE COMPARE_VALUE=0
GROUP BY TRUNC(FECHA, 'MM')
ORDER BY TRUNC(FECHA, 'MM') deSC;
;
