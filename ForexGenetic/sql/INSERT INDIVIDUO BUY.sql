...
CREATE TABLE TEMP_INDIVIDUO AS 
  SELECT (TRUNC(SYSDATE) - TO_DATE('1970/01/01', 'YYYY/MM/DD'))*24*60*60*1000||'.'||(ROWNUM) NUEVO_INDIVIDUO, IND.* 
  FROM INDIVIDUO IND
  WHERE --IND.ID LIKE '1454196071797.%'
  --AND 
  EXISTS (
    SELECT 1 FROM OPERACION_CONSOLIDADAS OPER 
    WHERE IND.ID = OPER.ID_INDIVIDUO AND OPER.PIPS>10000
  )
  --AND EXISTS (MINIMO N INDICADORES)
