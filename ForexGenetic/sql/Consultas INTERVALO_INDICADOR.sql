...
SELECT ID_INDICADOR, TIPO,
  ...
  ROUND(AVG(II.INTERVALO_SUPERIOR-II.INTERVALO_INFERIOR),5) DIFFAVG
  FROM INDICADOR_INDIVIDUO_BORRADO II 
    INNER JOIN INDIVIDUO_BORRADO IND ON IND.ID=II.ID_INDIVIDUO AND IND.TIPO_BORRADO='CANTIDAD_LIMITE'
  --WHERE II.TIPO='OPEN'
GROUP BY ID_INDICADOR, TIPO
ORDER BY ID_INDICADOR ASC, TIPO DESC
;

SELECT ROUND((II.INTERVALO_SUPERIOR-II.INTERVALO_INFERIOR),5) DIFFMAX, II.*
--DISTINCT II.ID_INDIVIDUO
FROM INDICADOR_INDIVIDUO II
WHERE II.ID_INDICADOR='BOLLINGER240' AND II.TIPO='OPEN'
AND II.INTERVALO_INFERIOR IS NOT NULL AND ROUND((II.INTERVALO_SUPERIOR-II.INTERVALO_INFERIOR),5)>=0.26853
--AND II.ID_INDIVIDUO='1477365411387.6555'
ORDER BY II.ID_INDIVIDUO
;

SELECT DISTINCT II.ID_INDIVIDUO
FROM INDICADOR_INDIVIDUO II
...
WHERE II.INTERVALO_INFERIOR IS NOT NULL AND 
  ((II_SINOPER.DIFFMIN<II_PROC.DIFFMIN AND ROUND((II.INTERVALO_SUPERIOR-II.INTERVALO_INFERIOR),5)<=II_SINOPER.DIFFMIN)
    OR (II_SINOPER.DIFFMAX>II_PROC.DIFFMAX AND ROUND((II.INTERVALO_SUPERIOR-II.INTERVALO_INFERIOR),5)>=II_SINOPER.DIFFMAX))
--AND II.ID_INDIVIDUO='1477365411387.6555'
ORDER BY II.ID_INDIVIDUO
;

SELECT DH.FECHA-1/24/60 FECHA   FROM DATOHISTORICO DH
				  INNER JOIN INDIVIDUO_INDICADORES IC ON IC.ID='1454889600000.104' AND 
				 ...
				 WHERE DH.FECHA>TO_DATE('20081031', 'YYYYMMDD') ORDER BY FECHA ASC 
         ;