...
FROM (
  ...
  FROM (
    ...
    LEFT JOIN OPERACION OPER ON SEMANAS.FECHA_SEMANA=TRUNC((OPER.FECHA_CIERRE),'IW') 
      AND OPER.ID_INDIVIDUO='1455148800000.531' AND OPER.FECHA_CIERRE IS NOT NULL
...
FROM (SELECT AC.ID_INDIVIDUO, AC.FECHA_SEMANA, AC.FECHA_CIERRE, AC.PIPS, AC.CANTIDAD, AC.MAXFE, AC.FECHA_HISTORICO,
  ...
  FROM ( SELECT MAX(OPER.ID_INDIVIDUO) OVER () ID_INDIVIDUO, SEMANAS.FECHA_SEMANA  FECHA_SEMANA, 
      ...
      NVL(SUM(OPER.PIPS) OVER (ORDER BY OPER.FECHA_CIERRE ASC RANGE BETWEEN INTERVAL '240' MONTH PRECEDING AND CURRENT ROW),0) PIPS_REGR,
      ...
      COUNT(OPER.PIPS) OVER (ORDER BY OPER.FECHA_CIERRE ASC RANGE BETWEEN INTERVAL '240' MONTH PRECEDING AND CURRENT ROW) CANTIDAD_REGR,
