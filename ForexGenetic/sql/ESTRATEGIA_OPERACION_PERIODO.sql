...
EOP.ID BETWEEN  1373953	AND 1384036
AND EOP.TIPO_OPERACION='BUY' AND EOP.FILTRO_PIPS_X_SEMANA=2000
...
ORDER BY EOP.FECHA_FINAL ASC
--ORDER BY EOP.PIPS_TOTALES DESC
;

SELECT PERI.ID, PRE_TFS.ID_INDIVIDUO, PRE_TFS.FECHA_SEMANA,
  ...
  WHERE 
	(PERI.PIPS_TOTALES>5000 AND PERI.ID BETWEEN 234084 AND 237029 --197004 AND 237029
  ...
  --AND PRE_TFS.ID_INDIVIDUO='1462763219878.42'
...
SELECT EOP.ID, PIPS_TOTALES PIPS, PIPS_TOTALES_PARALELAS PIPS_PARAL, ROUND(CANTIDAD_PARALELAS/(CANTIDAD+1),2) CANT, FILTRO_PIPS_X_SEMANA FILTR_SEM, EOP.* 
  FROM ESTRATEGIA_OPERACION_PERIODO EOP WHERE TIPO_OPERACION='BUY'
  AND ID BETWEEN 234084 AND 237029
...
SELECT PRE_TFS.ID_INDIVIDUO, PRE_TFS.FECHA_SEMANA
  FROM PREVIO_TOFILESTRING PRE_TFS
	WHERE
		--( --NVL(PRE_TFS.PIPS_SEMANA,0)>500 AND 
    --PRE_TFS.PIPS_MES>1400
		--AND PRE_TFS.PIPS_ANYO>-800 AND PRE_TFS.PIPS_TOTALES>0
      --)      AND 
      --PRE_TFS.TIPO_OPERACION='BUY'    AND 
      PRE_TFS.FECHA_SEMANA BETWEEN TO_DATE('2015/07/02 02:41', 'YYYY/MM/DD HH24:MI') 
        AND ADD_MONTHS(TO_DATE('2015/07/02 02:41', 'YYYY/MM/DD HH24:MI'),1)
;
