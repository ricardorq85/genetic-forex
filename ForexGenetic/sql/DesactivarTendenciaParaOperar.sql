...
--WHERE ACTIVA=0 AND FECHA_BASE>TO_DATE('2016/01/01 00:00', 'YYYY/MM/DD HH24:MI')
--AND PERIODO NOT LIKE 'EXT%'
--AND EXISTS (	SELECT TOP2.FECHA_BASE FROM TENDENCIA_PARA_OPERAR TOP2
--WHERE TOP2.FECHA_BASE=TOP.FECHA_BASE AND TOP2.PERIODO='EXTEXT'
--	AND TOP2.TIPO_OPERACION=TOP.TIPO_OPERACION);

UPDATE TENDENCIA_PARA_OPERAR SET ACTIVA=0 WHERE PERIODO='EXTSINF' AND ACTIVA=1;
UPDATE TENDENCIA_PARA_OPERAR SET ACTIVA=0 WHERE PERIODO='EXTEXT' AND ACTIVA=1;
COMMIT;
exit;

--UPDATE TENDENCIA_PARA_OPERAR TOP SET ACTIVA=0
--WHERE ACTIVA=1 AND FECHA_BASE>TO_DATE('2016/01/01 00:00', 'YYYY/MM/DD HH24:MI')
...
--UPDATE TENDENCIA_PARA_OPERAR SET ACTIVA=1 WHERE PERIODO='EXTEXT' AND ACTIVA=0;
...
WHERE FECHA_BASE>TO_DATE('2016/01/01 00:00', 'YYYY/MM/DD HH24:MI')
AND NOT EXISTS (	SELECT TOP2.FECHA_BASE FROM TENDENCIA_PARA_OPERAR TOP2
WHERE TOP2.FECHA_BASE=TOP.FECHA_BASE AND TOP2.PERIODO='EXTEXT'
	AND TOP2.TIPO_OPERACION=TOP.TIPO_OPERACION
)
