...
public class OperacionHelper {
	...
	public static List<Order> operacionesIndividuo(ResultSet resultado) throws SQLException {
		...
		while (resultado.next()) {
			Order order = operacion(resultado);
			list.add(order);
		}
		return list;
	}
	...
	public static Order operacion(ResultSet resultado) throws SQLException {
		Order order = new Order();
		order.setLot(resultado.getDouble("LOTE"));
		...
		if (resultado.getObject("MAX_FECHA_RETROCESO") != null) {
			order.setMaxFechaRetroceso(new Date(resultado.getTimestamp("MAX_FECHA_RETROCESO").getTime()));
		}

		return order;
	}

	public static List<DateInterval> vigencias(ResultSet resultado) throws SQLException {
		List<DateInterval> list = new ArrayList<DateInterval>();
		while (resultado.next()) {
			DateInterval interval = new DateInterval();
			interval.setLowInterval(new Date(resultado.getTimestamp("VIGENCIA1").getTime()));
			interval.setHighInterval(new Date(resultado.getTimestamp("VIGENCIA2").getTime()));
