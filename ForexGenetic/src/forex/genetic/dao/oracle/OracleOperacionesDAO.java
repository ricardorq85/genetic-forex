...
public class OracleOperacionesDAO extends OracleGeneticDAO<Order> implements IOperacionesDAO<Order> {
	...
	private static final String sqlEstadisticas = "WITH " + "PARAMETROS AS " + "	(SELECT "
			+ "		? AS PARAM_FECHA, " + "		? AS PARAM_DURACION, " + "		? AS PARAM_RETROCESO " + "	FROM "
			+ "		DUAL " + "	) " + "  SELECT IND.ID ID_INDIVIDUO,  "
			...
			+ "    COUNT(*) CANTIDAD " + "    FROM OPERACION OPER, PARAMETROS PARAM "
			...
			+ "    AND (PARAM.PARAM_RETROCESO IS NULL OR " + "      (OPER.PIPS<0 AND PARAM.PARAM_RETROCESO/1>0  "
			...
			+ "    COUNT(*) CANTIDAD " + "    FROM OPERACION P, PARAMETROS PARAM  " + "    WHERE P.PIPS>0  "
			...
			+ "    AND ((PARAM.PARAM_RETROCESO IS NULL) " + "     OR (PARAM.PARAM_RETROCESO>0 "
			+ "      AND (P.PIPS>=PARAM.PARAM_RETROCESO)) " + "     OR (PARAM.PARAM_RETROCESO<0 "
			...
			+ "    STDDEV(WEEK_MINUTES(FECHA_CIERRE,FECHA_APERTURA)) DUR_DESV, "
			+ "    MIN(MAX_PIPS_RETROCESO) PIPS_RETROCESO_MINIMO, MAX(MAX_PIPS_RETROCESO) PIPS_RETROCESO_MAXIMO, AVG(MAX_PIPS_RETROCESO) PIPS_RETROCESO_PROMEDIO, STATS_MODE(MAX_PIPS_RETROCESO) PIPS_RETROCESO_MODA,     "
			+ "    COUNT(*) CANTIDAD " + "    FROM OPERACION N, PARAMETROS PARAM " + "		WHERE N.PIPS<=0  "
			...
			+ "    AND ((PARAM.PARAM_RETROCESO IS NULL) " + "      OR (PARAM.PARAM_RETROCESO<0 "
			+ "        AND(N.PIPS<=PARAM.PARAM_RETROCESO)) " + "     OR (PARAM.PARAM_RETROCESO>0 "
			+ "      AND (N.MAX_PIPS_RETROCESO>=NVL(PARAM.PARAM_RETROCESO,0))))		 "
			+ "  GROUP BY N.ID_INDIVIDUO) NEGATIVOS ON NEGATIVOS.ID_INDIVIDUO=IND.ID " + " WHERE IND.ID=? "
			+ " GROUP BY IND.ID";
	 ...
	 */
	public Estadistica consultarEstadisticas2(Individuo individuo,
	...
	public Estadistica consultarEstadisticas(Individuo individuo,
		...
		try {
			int count = 1;
			stmtConsulta = this.connection.prepareStatement(sql);
			stmtConsulta.setTimestamp(count++, new Timestamp(parametroConsultaEstadistica.getFecha().getTime()));

			if (parametroConsultaEstadistica.getDuracion() != null) {
				stmtConsulta.setLong(count++, parametroConsultaEstadistica.getDuracion());
			} else {
				stmtConsulta.setNull(count++, java.sql.Types.INTEGER);
			}
			if (parametroConsultaEstadistica.getRetroceso() != null) {
				stmtConsulta.setDouble(count++, parametroConsultaEstadistica.getRetroceso());
			} else {
				stmtConsulta.setNull(count++, java.sql.Types.DECIMAL);
			}
			stmtConsulta.setString(count++, parametroConsultaEstadistica.getIndividuo().getId());
			resultado = stmtConsulta.executeQuery();
