 ...
 * @author ricardorq85
 */
public class ForexBorrarStopLossInconsistente {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException, SQLException {
        ...
        load().join();
        logTime("BorrarStopLossInconsistente: " + id, 1);
        String name = getPropertyString(LOG_PATH)
                + "BorrarStopLossInconsistente_"
                + id + "_log.log";
        ...
        BorradoManager manager = new BorradoInconsistentesStopLossManager();
        manager.borrarIndividuos();
        logTime("Fin: " + id, 1);
    }
    private static final Logger LOG = Logger.getLogger(ForexBorrarStopLossInconsistente.class.getName());
}
