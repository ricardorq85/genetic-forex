 ...
 * @author ricardorq85
 */
public class ForexBorrarIndividuosSinOperaciones {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException, SQLException {
        ...
        load().join();
        logTime("ForexBorrarIndividuosSinOperaciones: " + id, 1);
        String name = getPropertyString(LOG_PATH)
                + "ForexBorrarIndividuosSinOperaciones_"
                + id + ".log";
        ...
        BorradoManager manager = new BorradoIndividuoSinOperacionesManager();
        manager.borrarIndividuos();
        logTime("Fin: " + id, 1);
    }
    private static final Logger LOG = Logger.getLogger(ForexBorrarIndividuosSinOperaciones.class.getName());
}
