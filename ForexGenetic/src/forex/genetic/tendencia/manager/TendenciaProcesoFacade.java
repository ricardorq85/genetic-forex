...
public abstract class TendenciaProcesoFacade implements IGeneticManager {
				...
				for (Individuo individuo : individuos) {
					Runnable runner = new Runnable() {
						...
						public void run() {
							...
							try {
								tendencia = TendenciaProcesoFacade.this.calcularTendencia(puntoTendencia, individuo);
								if (tendencia != null) {
									LogUtil.logTime("Guardando..." + individuo.getId(), 4);
									LogUtil.logTime(tendencia.toString(), 2);
									LogUtil.logAvance(1);
									TendenciaProcesoFacade.this.guardarTendencia(tendencia);
