...
public class ProcesarTendenciasBuySellManager extends ProcesarTendenciasGraficaManager {
	...
	private TendenciasManager tendenciasManager;
	...
	private boolean parametroActualizarTendencia;

	public ProcesarTendenciasBuySellManager() throws ClassNotFoundException, SQLException {
		conn = JDBCUtil.getConnection();

		operacionManager = new OperacionesManager();
		tendenciasManager = new TendenciasManager();
		...
		parametroActualizarTendencia = Boolean.parseBoolean(parametroDAO.getValorParametro("SN_UPDATE_TENDENCIA"));
	}

	public Order procesarTendencias(Date fechaBase) {
		Order orden = new Order();
		orden.setTipo(OperationType.BUY);
		orden.setPips(100);
		orden.setTakeProfit(15.0D);
		orden.setStopLoss(25.0D);
		return orden;
	}

	public void procesarTendenciasOld() throws ClassNotFoundException, SQLException, ParseException, GeneticException {
		conn = JDBCUtil.getConnection();
...

		{
			...
			if (fechaProceso == null) {
				...
				Date nextFechaBase = (ultimaFechaApertura == null) ? tendenciaDAO.nextFechaBase(fechaProceso)
						: tendenciaDAO.nextFechaBase(ultimaFechaApertura);
				/*
				 * if ((individuo.getCurrentOrder() == null) &&
				 * (procesoTendencia == null)) { fechaProceso = nextFechaBase; }
				 * else {
				 */
				Date fechaHistorico = datoHistoricoDAO
						.getFechaHistoricaMinima(DateUtil.adicionarMinutos(fechaProceso, step - 1));
			...
			if (individuo.getCurrentOrder() != null) {
				...
				if (fechaCierre != null) {
					...
					// tendenciaDAO.nextFechaBase(DateUtil.adicionarMinutos(fechaCierre,
					// -1));
					fechaCierre = null;
				}
			}
			...
			if ((ultimaFechaProceso == null)
					|| (((maxMinFechaProceso.getHighInterval() - maxMinFechaProceso.getLowInterval())
							* PropertiesManager.getPairFactor()) > 100)) {
				...
				if ((actualizarTendencia) || (individuo.getCurrentOrder() == null)) {
					...
					analyzeProcesoTendencia = analyzeTendencia(procesoTendenciaList,
							((individuo.getCurrentOrder() == null) ? null
									: individuo.getCurrentOrder().getTipo().equals(Constants.OperationType.BUY)));
				...
				if ((procesoTendencia != null)
					...
					if ((points != null) && !(points.isEmpty())) {
						...
						if (point.getDate().equals(fechaProceso)) {
							...
							// if
							// (Math.abs(ultimoPrecioApertura-precioApertura))
							byLow = (precioApertura < procesoTendencia.getValorMasProbable());
							LogUtil.logTime("Precio Apertura=" + precioApertura + ";" + "Precio calculado="
									+ procesoTendencia.getValorMasProbable() + ";Precio calculado base="
									+ procesoTendencia.getValorMasProbable() + ";Pips calculados="
									+ NumberUtil.round(precioApertura - procesoTendencia.getValorMasProbable())
											* PropertiesManager.getPairFactor()
									+ ";Pips calculados base="
									+ NumberUtil.round(precioApertura - procesoTendencia.getValorMasProbable())
											* PropertiesManager.getPairFactor(),
									1);
						} else {
					...
					if ((procesoTendencia != null)) {
						if ((individuo.getCurrentOrder() != null)) {
							...
							if (!actualizarTendencia) {
								...
								double pipsActuales = operacionManager.calcularPips(Collections.singletonList(point), 0,
										order);
								LogUtil.logTime("pipsActuales=" + pipsActuales, 1);
								double tpTmp;
								double slTmp;
								tpTmp = Math.abs(procesoTendencia.getPipsMasProbable())
										* procesoTendencia.getProbabilidad() + pipsActuales;
								slTmp = Math.max(tpTmp / 2, analyzeProcesoTendencia.getStopLoss() + pipsActuales);
								if ((tpTmp < order.getOpenSpread())
										|| (((order.getTipo().equals(Constants.OperationType.BUY))
												&& (procesoTendencia.getPipsMasProbable() > 0))
												|| ((order.getTipo().equals(Constants.OperationType.SELL))
														&& (procesoTendencia.getPipsMasProbable() < 0)))) {
							...
							tp = Math.abs(procesoTendencia.getPipsMasProbable()) * procesoTendencia.getProbabilidad()
									- point.getSpread();
	...
	private AnalyzeProcesoTendencia analyzeTendencia(List<ProcesoTendencia> procesoTendenciaList, Boolean byLow) {
		...
		for (ProcesoTendencia procesoTendencia : procesoTendenciaList) {
			...
			if ((Math.abs(procesoTendencia.getPipsMasProbable()
					* procesoTendencia.getProbabilidad()) > (Constants.MIN_PIPS_MOVEMENT))
					&& (procesoTendencia.getCantidad() >= Constants.MIN_CANTIDAD_TENDENCIA_BASE)
					&& (procesoTendencia.getProbabilidad() >= Constants.MIN_PERCENT_PROB)) {
				/*
				 * if ((procesoTendenciaOperacion == null) ||
				 * ((procesoTendencia.getProbabilidad() >
				 * procesoTendenciaOperacion.getProbabilidad()) &&
				 * (procesoTendencia.getPipsMasProbable() /
				 * procesoTendenciaOperacion.getPipsMasProbable() > 0))) { if
				 * ((byLow == null) || ((byLow &&
				 * procesoTendencia.getPipsMasProbable() < 0) || (!byLow &&
				 * procesoTendencia.getPipsMasProbable() > 0))) {
				 * procesoTendenciaOperacion = procesoTendencia; } }
				 */
				if ((procesoTendencia.getPipsMasProbable() > 0)
						&& ((procesoTendenciaPositivos == null) || ((procesoTendencia.getProbabilidad()
								* procesoTendencia.getPipsMasProbable()) > (procesoTendenciaPositivos.getProbabilidad()
										* procesoTendenciaPositivos.getPipsMasProbable())))) {
					procesoTendenciaPositivos = procesoTendencia;
				}
				if ((procesoTendencia.getPipsMasProbable() < 0)
						&& ((procesoTendenciaNegativos == null) || ((procesoTendencia.getProbabilidad()
								* -procesoTendencia.getPipsMasProbable()) > (procesoTendenciaNegativos.getProbabilidad()
				 ...
				 * (Math.abs(procesoTendencia.getPipsMasProbable() *
				 * procesoTendencia.getProbabilidad()) >
				 * Math.abs(procesoTendenciaPositivos.getPipsMasProbable() *
				 * procesoTendenciaPositivos.getProbabilidad())))) {
				 * procesoTendenciaPositivos = procesoTendencia; stopPositivos =
				 * Math.max(stopPositivos,
				 * Math.abs(procesoTendencia.getPipsMasProbable() * (1 +
				 * procesoTendencia.getProbabilidad()))); } else if
				 * ((procesoTendencia.getPipsMasProbable() < 0) &&
				 ...
				 * procesoTendenciaNegativos = procesoTendencia; stopNegativos =
				 * Math.max(stopNegativos,
				 * Math.abs(procesoTendencia.getPipsMasProbable() * (1 +
				 * procesoTendencia.getProbabilidad()))); }
				 */
			}
			...
			if (byLow != null) {
				...
				if ((procesoTendenciaPositivos != null) && (procesoTendenciaNegativos == null)) {
					...
					if (((procesoTendenciaPositivos.getProbabilidad()
							* procesoTendenciaPositivos.getPipsMasProbable()) > (procesoTendenciaNegativos
									.getProbabilidad() * -procesoTendenciaNegativos.getPipsMasProbable()))
							&& (((1 + procesoTendenciaNegativos.getProbabilidad())
									* procesoTendenciaPositivos.getProbabilidad()
									* procesoTendenciaPositivos
											.getPipsMasProbable()) <= ((1 + procesoTendenciaNegativos.getProbabilidad())
													* -procesoTendenciaNegativos.getPipsMasProbable()))) {
						procesoTendenciaOperacion = procesoTendenciaPositivos;
						stopNegativos = -(1 + procesoTendenciaNegativos.getProbabilidad())
								* procesoTendenciaNegativos.getPipsMasProbable();
						stopPositivos = 0.0D;
							...
							* -procesoTendenciaNegativos.getPipsMasProbable()) > (procesoTendenciaPositivos
									.getProbabilidad() * procesoTendenciaPositivos.getPipsMasProbable()))
							&& (((1 + procesoTendenciaPositivos.getProbabilidad())
									* procesoTendenciaNegativos.getProbabilidad()
									* -procesoTendenciaNegativos.getPipsMasProbable()) <= ((1
											+ procesoTendenciaPositivos.getProbabilidad())
											* procesoTendenciaPositivos.getPipsMasProbable()))) {
						procesoTendenciaOperacion = procesoTendenciaNegativos;
						stopPositivos = (1 + procesoTendenciaPositivos.getProbabilidad())
		...
		if (procesoTendenciaOperacion != null) {
			 ...
			 * ((procesoTendenciaNegativos == null) ||
			 * Math.abs(procesoTendenciaOperacion.getPipsMasProbable()) >
			 * Math.abs(procesoTendenciaNegativos.getPipsMasProbable()))) ||
