		...
		while ((points != null) && (!points.isEmpty()) && ((fechaFin == null) || fechaFin.after(fechaProceso))) {
			for (int j = 0; (j < points.size()
				...
				for (int i = 0; i < individuos.size(); i++) {
					...
					conn.commit();
					Estadistica estadistica = operacionesDAO.consultarEstadisticas(individuo, null);
					...
					if (true) {
						...
						conn.commit();
						Estadistica estadisticaActual = operacionesDAO.consultarEstadisticas(individuo, null);
						for (int k = -1; k < 2; k += 2) {
